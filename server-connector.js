  ...
  for (const location of CONFIG_LOCATIONS) {
    try {
      ...
      if (config.servers && Array.isArray(config.servers)) {
        console.log(`Loaded server configs from ${location}`);
        return config.servers;
      }
    } catch (error) {
      if (error.code !== 'ENOENT') {
        console.warn(`Error reading config from ${location}:`, error.message);
      }
    }
  }
  
  // No valid configs found, use defaults
  console.log('Using default server configurations');
  ...
  try {
    ...
    const response = await fetch(`${server.url}/tools`, {
      headers: {
        ...
        'X-API-Key': server.apiKey || process.env[`${server.id.toUpperCase()}_API_KEY`]
      }
    });
    ...
    console.error(`Error fetching tools from ${server.url}:`, error.message);
  ...
  switch (server.category) {
    ...
    case 'github':
      tools.push(
        ...
        createTool('github_create_pull_request', 'Create Pull Request', 'Create a new pull request', server.category)
      );
      break;
      
    case 'memory':
      tools.push(
        createTool('memory_create_entities', 'Create Entities', 'Create new entities in knowledge graph', server.category),
        createTool('memory_create_relations', 'Create Relations', 'Create relations between entities', server.category),
        createTool('memory_search_nodes', 'Search Knowledge Graph', 'Search for nodes in knowledge graph', server.category),
        createTool('memory_read_graph', 'Read Knowledge Graph', 'Read the entire knowledge graph', server.category),
        createTool('memory_delete_entities', 'Delete Entities', 'Delete entities from knowledge graph', server.category)
      );
      break;
      
    case 'terminal':
      tools.push(
        createTool('win_cli_execute_command', 'Execute Command', 'Execute a command in specified shell', server.category),
        createTool('win_cli_get_command_history', 'Get Command History', 'Get history of executed commands', server.category),
        createTool('win_cli_ssh_execute', 'Execute SSH Command', 'Execute command on remote host via SSH', server.category),
        createTool('win_cli_get_current_directory', 'Get Current Directory', 'Get current working directory', server.category)
      );
      break;
      
    case 'ai':
      tools.push(
        createTool('sequential_thinking', 'Sequential Thinking', 'Break down complex problems with step-by-step thinking', server.category),
        createTool('web_search', 'Web Search', 'Search the web for information', server.category)
      );
      break;
    ...
    default:
      ...
      tools.push(
        createTool(`${server.id}_tool1`, 'Generic Tool 1', 'Generic tool description', server.category),
        createTool(`${server.id}_tool2`, 'Generic Tool 2', 'Generic tool description', server.category)
      );
  }
