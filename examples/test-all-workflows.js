...
function displayExecutionStatus(status) {
  ...
  const completedPercentage = Math.round((status.completedSteps.length / totalSteps) * 100);
 ...
 */
async function pollExecutionUntilComplete(executionId, interval = 2000, timeout = 600000) {
...
async function getWorkflowContext(workflow) {
  ...
  if (workflowId.includes('research') || workflowId.includes('web-research')) {
    ...
    const language = await question('Enter programming language (default: javascript): ');
