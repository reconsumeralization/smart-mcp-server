...
async function runBasicChatExample() {
  ...
  const result = await chatSession.sendMessage("Explain the key differences between Gemini 1.5 and Gemini 2.5 models");
...
async function runInlineDataExample() {
  ...
  const prompt = "Create a simple diagram showing the evolution of AI models from GPT to Gemini";
      ...
      for (let partIndex = 0; partIndex < candidates[candidateIndex].content.parts.length; partIndex++) {
        ...
        if (part.inlineData) {
          try {
            ...
            const filename = path.join(outputDir, `gemini_output_${candidateIndex}_${partIndex}.${extension}`);
            await fs.writeFile(filename, Buffer.from(part.inlineData.data, 'base64'));
...
async function runToolCallingExample() {
  ...
  const prompt = "I'm planning a trip to Tokyo next week. What's the weather like there? Also, if I have a budget of $1500 for a 5-day trip, how much can I spend per day?";
  console.log(`Prompt: "${prompt}"`);
  ...
  if (result.response.candidates && 
    ...
    const secondFollowUp = await chat.sendMessage("Could you also calculate how much that would be in Japanese yen, if the exchange rate is 1 USD = 155 JPY?");
