...
import {
  ...
  getToolRecommendations as _getToolRecommendations,
...
function formatTool(tool) {
  const usageInfo =
    tool.usageCount !== undefined ? ` (Usage: ${tool.usageCount})` : '';
  const score =
    tool.relevanceScore !== undefined
      ? ` (Score: ${tool.relevanceScore.toFixed(2)})`
      : '';
...
function demonstrateUsageFrequency() {
  ...
  console.log(
    'Recorded usage patterns: read_file (5), edit_file (3), web_search (8), github (2)'
  );
...
function demonstrateWeightCustomization() {
  ...
  const defaultRecommendations = selectToolsForContext(
    availableTools,
    query,
    3
  );
  ...
  const modifiedRecommendations = selectToolsForContext(
    availableTools,
    query,
    3
  );
