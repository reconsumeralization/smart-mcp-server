...
async function generateTextExample() {
  ...
  const prompt =
    'What are three major benefits of using AI for communication protocols?';
...
async function chatExample() {
  ...
  try {
    ...
    // First message
    console.log(
      `User: Hello, I'd like to learn about Model Context Protocols.`
    );
...
async function imageInputExample() {
  ...
  if (!fs.existsSync(imagePath)) {
    console.log(
      `Note: Image example skipped. Place a test image at: ${imagePath}`
    );
    return;
  }
...
async function customConfigExample() {
  ...
  try {
    ...
    const prompt =
      'Generate a brief, structured protocol for integrating a database with an AI model.';
...
async function runExamples() {
  console.log(
    '[' + new Date().toISOString() + '] Starting Gemini API examples...'
  );
  ...
  try {
    ...
    if (!isValid) {
      throw new Error(
        'Invalid or missing Gemini API key. Set GEMINI_API_KEY environment variable.'
      );
    }
    ...
    console.error(
      '[' + new Date().toISOString() + '] Error running examples:',
      error
    );
  }
}
