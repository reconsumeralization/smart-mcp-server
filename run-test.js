...
function startServer() {
  ...
  serverProcess.stdout.on('data', (data) => {
    console.log(
      `${colors.server}[SERVER] ${data.toString().trim()}${colors.reset}`
    );
  });

  serverProcess.stderr.on('data', (data) => {
    console.error(
      `${colors.error}[SERVER ERROR] ${data.toString().trim()}${colors.reset}`
    );
  });

  serverProcess.on('close', (code) => {
    console.log(
      `${colors.info}Server process exited with code ${code}${colors.reset}`
    );
    serverProcess = null;
  ...
  return new Promise((resolve) => {
    ...
    const serverStartTimeout = setTimeout(() => {
      console.log(
        `${colors.info}Server startup timeout - assuming it's ready${colors.reset}`
      );
      resolve();
    }, 5000);
    ...
    serverProcess.stdout.on('data', (data) => {
      if (data.toString().includes('Server running on port')) {
        ...
        console.log(
          `${colors.info}Server is ready. Starting client...${colors.reset}`
        );
        resolve();
      }
    });
...
function startClient() {
  ...
  clientProcess.on('close', (code) => {
    console.log(
      `${colors.info}Client process exited with code ${code}${colors.reset}`
    );
    clientProcess = null;
    ...
    if (serverProcess) {
      rl.question(
        'Client closed. Do you want to restart the client? (y/n) ',
        (answer) => {
...
async function runTest() {
  try {
    ...
    console.error(
      `${colors.error}Error running test: ${error.message}${colors.reset}`
    );
    cleanup();
    rl.close();
  }
...
runTest().catch((error) => {
  console.error(
    `${colors.error}Unhandled error: ${error.message}${colors.reset}`
  );
  cleanup();
  rl.close();
  process.exit(1);
