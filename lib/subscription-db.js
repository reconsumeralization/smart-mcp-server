...
async function insertSubscription(subscription) {
  try {
    const { id, customer_id, stripe_subscription_id, status, trial_end_date, next_invoice_date, plan_details } = subscription;
    const insertQuery = `
      INSERT INTO subscriptions (id, customer_id, stripe_subscription_id, status, trial_end_date, next_invoice_date, plan_details)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *;
    `;
    const { rows } = await query(insertQuery, [id, customer_id, stripe_subscription_id, status, trial_end_date, next_invoice_date, plan_details]);
    logger.info('Subscription inserted:', { subscriptionId: id });
    return rows[0];
  } catch (error) {
    logger.error('Error inserting subscription:', { error: error.message, subscription });
    throw error;
  }
}
