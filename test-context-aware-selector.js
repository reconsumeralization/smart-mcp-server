import { selectToolsForContext, recordToolUsage, getToolsByCategory, getToolRecommendations } from './context-aware-selector.js';
...
const mockTools = [
  ...
  { id: 'codebase_search', name: 'Codebase Search', description: 'Searches the codebase' },
  { id: 'grep_search', name: 'Grep Search', description: 'Searches using regex' },
  { id: 'run_terminal_cmd', name: 'Run Terminal Command', description: 'Executes a command' },
  { id: 'mcp_win_cli_execute_command', name: 'Execute Command', description: 'Executes a command' },
  { id: 'mcp_memory_create_entities', name: 'Create Memory Entities', description: 'Creates memory entities' },
  { id: 'mcp_memory_create_relations', name: 'Create Memory Relations', description: 'Creates memory relations' },
  { id: 'mcp_github_create_repository', name: 'Create GitHub Repository', description: 'Creates a GitHub repository' },
  { id: 'mcp_github_push_files', name: 'Push Files', description: 'Pushes files to a repository' }
];
...
const contexts = [
  {
    ...
    userMessage: 'I need to edit the README.md file to add installation instructions',
    expectedTopTools: ['edit_file', 'read_file']
  },
  {
    id: 'context-2',
    description: 'GitHub context',
    userMessage: 'Create a new repository for this project and push the initial commit',
  ...
  },
  {
    ...
    userMessage: 'Save information about the server architecture to the knowledge graph',
    expectedTopTools: ['mcp_memory_create_entities', 'mcp_memory_create_relations']
  },
  {
    id: 'context-5',
...
function testContextSelector() {
  ...
  contexts.forEach((context, index) => {
    ...
    const selectedTools = selectToolsForContext(mockTools, context.userMessage, 5);
    
    console.log('\nSelected tools:');
    if (selectedTools && selectedTools.length > 0) {
      selectedTools.forEach((tool, i) => {
        console.log(`${i + 1}. ${tool.id} (Score: ${tool.relevance ? tool.relevance.toFixed(2) : 'N/A'})`);
      });
    } else {
    ...
    const foundExpected = context.expectedTopTools.filter(t => selectedToolIds.includes(t));
    
    console.log('\nExpected tools found:', foundExpected.length, 'of', context.expectedTopTools.length);
    if (foundExpected.length < context.expectedTopTools.length) {
      const missing = context.expectedTopTools.filter(t => !selectedToolIds.includes(t));
